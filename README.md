...
Discord テキストチャンネルのメッセージを **完全ネイティブ実装のメモリベース音声処理** でリアルタイム読み上げするクロスプラットフォーム対応アプリケーションです。

## ✨ 特徴

- 🚀 **完全ネイティブ実装**: 外部音声ライブラリ不要の高速処理
- 🎯 **ゼロファイル処理**: 一時ファイル作成完全排除
...
### 完全ネイティブ実装による革新的改善
- ✅ **ゼロ依存**: `play-sound` ライブラリ完全排除
- ✅ **超高速**: ディスクI/O完全排除 + ネイティブAPI直接利用
- ✅ **完全メモリ**: 音声データは全てメモリ内処理
- ✅ **一時ファイル排除**: `temp_audio` ディレクトリ作成完全不要

### プラットフォーム別ネイティブ最適化
- **Windows**: PowerShell MediaPlayer MemoryStream による完全メモリ再生
- **macOS**: afplay stdin による完全メモリ再生 (PCM/WAV/AIFF対応)
- **VOICEVOX**: Buffer直接変換による完全メモリ処理

### 従来の問題点（完全解決）
- ❌ 外部音声ライブラリ依存 → ✅ **完全ネイティブ実装**
- ❌ 一時ファイルの作成・削除による遅延 → ✅ **完全排除**
- ❌ ディスク容量の消費 → ✅ **完全排除**
- ❌ ファイルI/Oによるボトルネック → ✅ **完全排除**
- ❌ temp_audioディレクトリの作成 → ✅ **完全排除**
- ❌ play-soundライブラリのオーバーヘッド → ✅ **完全排除**
...
| `zundamon_tuntun` | ずんだもん | セクシー | 5 |
| `metan` | 四国めたん | ノーマル | 2 |
| `metan_amaama` | 四国めたん | あまあま | 0 |
| `tsumugi` | 春日部つむぎ | ノーマル | 8 |
| `ryusei` | 青山龍星 | ノーマル | 13 |
| `hau` | 雨晴はう | ノーマル | 10 |

> 📋 **完全な話者一覧**: [VOICEVOX_SPEAKERS.md](./VOICEVOX_SPEAKERS.md) (総99人)  
> 🔄 **最新情報**: `npm run generate-speakers` で更新
...
5. **完全ネイティブ処理エラー**
   - システムの空きメモリを確認
   - PowerShell実行ポリシーを確認（Windows）
   - afplayコマンドの可用性を確認（macOS）
...
**🎉 完全ネイティブ実装により、最高速のメモリベース処理で究極にスムーズな配信体験をお楽しみください！**